...
def insertByInstance(instance, label, value):
 	
	sparqlSesame = SPARQLWrapper("http://localhost:8080/rdf4j-server/repositories/SocialNetwork/statements")
	queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX sn:  <http://ciff.curso2015/ontologies/owl/socialNetwork#> INSERT DATA { sn:" + instance +  " a sn:Person; rdfs:"+label+" '"+ value +"' . }"
	#print "sql", queryString

	sparqlSesame.setQuery(queryString)
	sparqlSesame.method = 'POST'
	#results = sparqlSesame.query()
	sparqlSesame.setReturnFormat(JSON)
	query   = sparqlSesame.query()
	results = query.convert()
...
def getDBpediaResource (label, lang, endpoint):
	...
	if (lang):
		...
		queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> SELECT ?s,?fecha  WHERE {?s dbo:birthYear ?fecha  . ?s rdfs:label \"" + label + "\"@" +lang + " . } "
	else:
		...
		#queryString = "" 
		queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> SELECT ?s,?fecha WHERE { ?s ?birthYear ?fecha  . ?s rdfs:label \"" + label + " . }"
	...
	for result in results["results"]["bindings"]:
		...
		fecha = result["fecha"]["value"]
		print "->The resource: " + resource
		print "->The fecha: " , fecha
		insertByInstance("instancia1","fecha",fecha)
...
def getLinkedmdbResource (label, lang, endpoint):
	...
	if (lang):
		...
		#queryString = "" 
		queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX movie: <http://data.linkedmdb.org/resource/movie/> SELECT ?s ?fecha WHERE { ?s movie:initial_release_date ?fecha . ?s rdfs:label \"" + label + "\"@" +lang + " . } "
		...
		#queryString = "" 
		queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX movie: <http://data.linkedmdb.org/resource/movie/>  SELECT ?s ?fecha WHERE { ?s movie:initial_release_date ?fecha . ?s rdfs:label \"" + label + "\"" "} "
	...
	for result in results["results"]["bindings"]:
		...
		print "->The resource: " + resource
		print "->The fecha: " , fecha
		insertByInstance("instancia2","fecha",fecha)
...
def getBNEResource (label, lang, endpoint):
	...
	if (lang):
		...
		#queryString = ""
		queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX ns2:<http://datos.bne.es/def/>  SELECT ?s ?fecha WHERE { ?s ns2:P5002 ?fecha . ?s rdfs:label \"" + label + "\"@" +lang + " . } "
		...
		#queryString = ""
		queryString = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX ns2: <http://datos.bne.es/def/>  SELECT ?s ?fecha WHERE { ?s ns2:P5002 ?fecha . ?s rdfs:label \"" + label + "\""  "}"
	...
	for result in results["results"]["bindings"]:
		...
		print "->The resource: " + resource
		print "->The fecha: " , fecha
		insertByInstance("instancia3","fecha",fecha)
